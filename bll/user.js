...
function validate(user, currentUser) {
	...
	return userDAL.findByName(user.username, user.nickname).then((result) => {
		return result.some((u) => { return u.userid != user.userid; }) ?
...
exports.create = (user, currentUser) => {
	const err = checkPassword(user.password);
	return err ?
		util.createError(err) :
		validate(user, currentUser).then(() => {
...
// 更新用户（密码除外）
// currentUser为当前操作用户，用于检查权限
exports.update = (user, userid, currentUser) => {
	if (!validator.isAutoId(userid)) { return util.createError('无效的用户编号'); }

	user.userid = userid;
	// 基本权限验证在页面完成
	return validate(user, currentUser).then(() => {
		return userDAL.update(user.toDbRecord(), userid);
	}).then(userGroupBLL.clearCache);
};

// 更新密码
exports.updatePassword = (newPassword, oldPassword, username) => {
	...
	return userDAL.readByUsername(username, oldPassword).then((result) => {
		if (!result) { return util.createError('用户不存在或旧密码错误'); }

		newPassword = encryptPassword(newPassword);
		return userDAL.updatePassword(newPassword, username).then((result) => {
...
// 更新最后在线时间、IP
const updateActivity = exports.updateActivity = (ip, userid) => {
...
exports.readByUserId = (userid) => {
	return validator.isAutoId(userid) ?
		userDAL.readByUserId(userid).then((result) => {
			if (result[0]) {
...
exports.readByUsernameAndPassword = (username, password) => {
	let err;
	...
	return err ?
		util.createError(err) :
		userDAL.readByUsername(username, password).then((result) => {
			if (result[0]) {
				return userModel.createEntity(result[0]);
			}
		});
...
exports.delete = (userids) => {
	...
	if (!userids.length) {
		err = '请指定要操作的用户';
	} else if (userids.some((id) => { return !validator.isAutoId(id); })) {
		err = '无效的用户编号';
	}
...
exports.list = (params, pageSize, page) => {
	...
	return userDAL.list(params, pageSize, page).then((result) => {
...
const loginRetryHistory = {
	...
	// 清理过期的历史数据
	clean: function() {
		for (let key in this._data) { this.check(key); }
	}
};

// 定时清理历史记录
setInterval(() => { loginRetryHistory.clean(); }, loginRetryHistory.LOCK_TIME);

// 登录
exports.login = (username, password, ip) => {
	let err;
	...
	if (err) {
		...
		return userDAL.readByUsername(username).then((result) => {
			result = result[0];
			if (result) {
			...
			if (err) {
				...
				// 更新用户最后活动时间和IP
				return updateActivity(ip, result.userid).then(() => {
